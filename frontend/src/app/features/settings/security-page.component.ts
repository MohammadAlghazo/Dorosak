  ...
  template: `
    <section class="settings-page" aria-labelledby="security-title">
      ...
      <article class="settings-card">
        ...
        >
          <label for="new-email">{{
            locale.locale() === 'ar' ? 'البريد الجديد' : 'New email address'
          }}</label>
          <input
            id="new-email"
            type="email"
            ...
            autocomplete="email"
            [attr.aria-invalid]="
              emailForm.controls.newEmail.touched && emailForm.controls.newEmail.invalid
            "
          />
          ...
          <input
            ...
            [attr.aria-invalid]="
              emailForm.controls.currentPassword.touched &&
              emailForm.controls.currentPassword.invalid
            "
          />
        ...
        >
          ...
          <input
            ...
            [attr.aria-invalid]="
              ...
              passwordForm.hasError('fieldsMismatch')
            "
            aria-describedby="new-password-confirm-error"
          />
          @if (
            passwordForm.controls.confirmPassword.touched && passwordForm.hasError('fieldsMismatch')
          ) {
      ...
      <article class="settings-card">
        ...
        @if (totpSetup()) {
          <div class="totp-details">
            <h3>
              {{ locale.locale() === 'ar' ? 'أكمل إعداد التطبيق' : 'Finish setting up your app' }}
            </h3>
            <p>
              {{
                locale.locale() === 'ar'
            ...
            <dl>
              <div>
                <dt>{{ locale.locale() === 'ar' ? 'المفتاح' : 'Secret' }}</dt>
                <dd>
                  <code>{{ totpSetup()?.secret }}</code>
                </dd>
              </div>
              <div>
                <dt>{{ locale.locale() === 'ar' ? 'رابط التطبيق' : 'App URI' }}</dt>
                <dd>
                  <code class="breakable">{{ totpSetup()?.otpAuthUri }}</code>
                </dd>
              </div>
            </dl>
            <form
              class="identity-form"
              [formGroup]="mfaConfirmForm"
              (ngSubmit)="confirmMfa()"
              novalidate
            >
              <label for="totp-code">{{
                locale.locale() === 'ar' ? 'رمز التطبيق' : 'Authenticator code'
              }}</label>
              <input
                id="totp-code"
                inputmode="numeric"
                ...
                [attr.aria-invalid]="
                  mfaConfirmForm.controls.code.touched && mfaConfirmForm.controls.code.invalid
                "
        ...
        @if (mfaEnabled()) {
          <form
            class="identity-form compact-form"
            [formGroup]="disableMfaForm"
            (ngSubmit)="disableMfa()"
            novalidate
          >
            <label for="disable-mfa-password">{{
              locale.locale() === 'ar'
                ? 'كلمة المرور لتعطيل التحقق'
                : 'Password to disable verification'
            }}</label>
            <input
              ...
              autocomplete="current-password"
              [attr.aria-invalid]="
                disableMfaForm.controls.currentPassword.touched &&
                disableMfaForm.controls.currentPassword.invalid
              "
...
export class SecurityPageComponent {
  ...
  protected disableMfa(): void {
    ...
    this.identityApi.disableMfa(this.disableMfaForm.controls.currentPassword.value).subscribe({
      next: () => {
