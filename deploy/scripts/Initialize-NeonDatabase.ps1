...
$runtimeTableResult = Invoke-DatabaseScalar -Connection $runtimeUri `
    -Sql "SELECT (to_regclass('operations.outbox_messages') IS NOT NULL) || '|' || has_table_privilege(current_user, 'operations.outbox_messages', 'SELECT') || '|' || has_table_privilege(current_user, 'operations.outbox_messages', 'TRUNCATE') || '|' || has_table_privilege(current_user, 'operations.schema_compatibility', 'SELECT') || '|' || has_table_privilege(current_user, 'operations.schema_compatibility', 'UPDATE');"
if ($runtimeTableResult -ne "true|true|false|true|false") {
